return {
	...
	init = function()
		local group = vim.api.nvim_create_augroup("viktor-obsidian-markdown", { clear = true })

		vim.api.nvim_create_autocmd("FileType", {
			group = group,
			pattern = "markdown",
			callback = function(args)
				require("viktor.core.obsidian").setup_markdown_buffer(args.buf)
			end,
		})
	end,
	opts = function()
		return require("viktor.core.obsidian").opts()
	end,
	config = function(_, opts)
		require("obsidian").setup(opts)
		require("viktor.core.obsidian").patch_template_substitutions()
	end,
}
